...
import { MIN_PREDICTION_PAYOUT_MULTIPLIER } from "@/lib/config"
...
export async function POST(
  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const totalPool = openSelections.reduce(
        (sum, selection) => sum + selection.amount,
        0
      )

      const winningSelections = openSelections.filter(
        (selection) => selection.outcome === resolvedOutcome
      )
      const losingSelections = openSelections.filter(
        (selection) => selection.outcome !== resolvedOutcome
      )

      const totalWinningAmount = winningSelections.reduce(
        (sum, selection) => sum + selection.amount,
        0
      )
      ...
      const winnerPayouts = winningSelections.map((selection) => {
        ...
        const minimumPayout =
          selection.amount * MIN_PREDICTION_PAYOUT_MULTIPLIER
      ...
      const totalPayoutTarget = floored.reduce(
        (sum, entry) => sum + entry.payoutAmount,
        0
      )
      let remainder = Math.ceil(
        winnerPayouts.reduce((sum, entry) => sum + entry.rawPayout, 0) -
          totalPayoutTarget
      )
