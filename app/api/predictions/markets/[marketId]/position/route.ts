...
export async function POST(
  ...
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      const openSelectionsForOutcome = openSelections.filter(
        (selection) => selection.outcome === outcomeEnum
      )
      const openSelectionsForOtherOutcome = openSelections.filter(
        (selection) => selection.outcome !== outcomeEnum
      )

      const currentOpenAmount = openSelectionsForOutcome.reduce(
        (sum, selection) => sum + selection.amount,
        0
      )
      const currentOpenAmountOnOtherOutcome =
        openSelectionsForOtherOutcome.reduce(
          (sum, selection) => sum + selection.amount,
          0
        )
      ...
      await closeSelectionAmount(
        tx,
        openSelectionsForOutcome,
        amount,
        parsedMarketId,
        player.id,
        now
