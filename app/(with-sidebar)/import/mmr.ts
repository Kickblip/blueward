import { FAVORED_ROLE_MMR_DEBUFF } from "@/lib/config"
...
export function calculateMMR(participant: any): number {
  ...
  /* --------------------------- primary impact -------------------------- */
  const carryPoints = tierScore(
    participant.challenges.teamDamagePercentage ?? 0,
    [
  ...
  const tankPoints = tierScore(
    participant.challenges.damageTakenOnTeamPercentage ?? 0,
    [
  ...
  const enchanterPoints = tierScore(
  ...
  points += clamp(
    Math.max(carryPoints, tankPoints, enchanterPoints, ccPoints),
    0,
    10
  )
  ...
  const objectiveDamagePoints = tierScore(
  ...
  const epicStealPoints =
    (participant.challenges.epicMonsterSteals ?? 0) >= 1 ? 10 : 0

  points += clamp(
    Math.max(turretPoints, platePoints, objectiveDamagePoints, epicStealPoints),
    0,
    10
  )

  /* ------------------------------ farming ------------------------------ */
  const laneCsPoints = tierScore(
  ...
  const jungleCsPoints = tierScore(
    participant.challenges.jungleCsBefore10Minutes ?? 0,
    [
  ...
  const visionWork =
    (participant.challenges.controlWardsPlaced ?? 0) +
    (participant.challenges.wardTakedowns ?? 0)
  ...
  points += clamp(
    Math.max(laneCsPoints, jungleCsPoints, supportVisionPoints),
    0,
    8
  )
