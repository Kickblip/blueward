import { fetchWithRetry, importMatchJson } from "../actions"
...
export default async function Import({
  ...
  const r = await fetchWithRetry(
    `${process.env.NEXT_PUBLIC_RIOT_RSO_API_ROOT}/matches/${matchId}`,
    {
  ...
  if (!matchData) {
    return (
      <ErrorMessage
        code={r.status}
        message="Failed to fetch match data from Riot API."
      />
    )
  }
  ...
  try {
    ...
    return (
      <ErrorMessage
        code={500}
        message={`DB import failed: ${toErrorMessage(err)}`}
      />
    )
  }
}
