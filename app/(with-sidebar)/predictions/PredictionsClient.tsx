...
import { getProjectedPredictionMultiplier } from "@/lib/config"
...
const isActive = ({ status }: Market) =>
  status === "OPEN" || status === "LOCKED"
const getDisplayStatus = (
  { status, locksAt }: Market,
  now: number
): "OPEN" | "LOCKED" =>
  status === "LOCKED" || (!!locksAt && new Date(locksAt).getTime() <= now)
    ? "LOCKED"
    : "OPEN"
...
const syncBalance = (balance: number) =>
  mutate("/api/shop/balance", { balance }, false)

export default function PredictionsClient() {
  const [markets, setMarkets] = useState<Market[]>([])
  const [selected, setSelected] = useState(0)
  const [buying, setBuying] = useState<Record<1 | 2, boolean>>({
    1: false,
    2: false,
  })
  ...
  const [submitting, setSubmitting] = useState<
    `buy-${1 | 2}` | `remove-${1 | 2}` | null
  >(null)

  const closeBuys = () => setBuying({ 1: false, 2: false })
  const setSideBuying = (id: 1 | 2, value: boolean) =>
    setBuying((b) => ({ ...b, [id]: value }) as Record<1 | 2, boolean>)

  const loadMarkets = async () => {
    try {
      setError(null)
      const res = await fetch("/api/predictions/markets/get", {
        cache: "no-store",
      })
  ...
  const msRemaining = selectedMarket.locksAt
    ? new Date(selectedMarket.locksAt).getTime() - now
    : 0
  const highestBet = Math.max(
    0,
    ...[...leftOrders, ...rightOrders].map(({ amount }) => amount)
  )
  ...
  const placeBet = async (outcome: 1 | 2) => {
    if (getDisplayStatus(selectedMarket, Date.now()) !== "OPEN")
      return setError("This market is no longer open")
    ...
    try {
      ...
      const res = await fetch(
        `/api/predictions/markets/${selectedMarket.id}/position`,
        {
          method: "POST",
  ...
  const removeBet = async (outcome: 1 | 2, amount: number) => {
    ...
    if (!Number.isInteger(amount) || amount <= 0)
      return setError("No open bet to remove")
        ...
        <div className="flex flex-col items-center gap-4 p-4">
          <div className="mb-2 flex w-full items-end justify-between text-left font-oswald text-3xl font-semibold uppercase">
            <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center gap-3 text-xs font-semibold text-zinc-500 uppercase">
                <span>{displayStatus}</span>
                {displayStatus === "OPEN" && (
                  <span>{countdown(msRemaining)}</span>
                )}
              </div>
            </div>
          ...
          {error && (
            <div className="w-full rounded-md bg-rose-500/10 px-3 py-2 text-sm text-rose-300">
              {error}
            </div>
          )}
          ...
          <div className="grid w-full grid-cols-2 gap-4">
            {[
              ...
              <OutcomeColumn
                ...
                onRemove={() =>
                  removeBet(id, id === 1 ? leftMyAmount : rightMyAmount)
                }
...
function OutcomeColumn({
  ...
  const canBuy =
    displayStatus === "OPEN" && (myOpenAmount > 0 || oppositeOpenAmount === 0)
  ...
  return (
    <div className="grid content-start gap-1">
      ...
      <div className="mt-2 mb-4 min-h-10">
          ...
          {isBuying ? (
            ...
            >
              <button
                ...
                disabled={!canBuy}
                className={`flex h-10 w-full items-center justify-center rounded-md px-3 py-1 font-oswald text-xl font-semibold uppercase transition-colors duration-200 disabled:cursor-not-allowed disabled:opacity-50 ${
                  left
      ...
      <p
        className={`mb-1 text-sm font-semibold ${left ? "text-left" : "text-right"}`}
      >
    ...
    >
      ...
      <input
        ...
        onChange={({ target: { value } }) =>
          setAmount(Number(value.replace(/\D/g, "")) || 0)
        }
        className="flex-1 px-3 py-1.5 font-oswald text-sm font-semibold uppercase outline-none"
      />
      <button
        type="button"
        onClick={onSubmit}
        disabled={isSubmitting || amount <= 0}
        className="cursor-pointer rounded-md px-1 py-1.5 font-oswald text-sm font-semibold uppercase transition-colors duration-200 hover:text-zinc-400 disabled:cursor-not-allowed disabled:opacity-50"
      >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        ...
        <span
          className={
            size === "small" ? "text-sm" : "font-oswald text-xl font-semibold"
          }
        >
          {toNumberWithCommas(value)}
        </span>
      </div>
      ...
      <div className="flex items-center gap-1">
        <CrystalIcon size={size === "small" ? 12 : 20} />
        <span
          className={`text-lime-400 ${size === "small" ? "text-sm" : "font-oswald text-xl font-semibold"} `}
        >
...
export function BuyerRow({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-md ${rtl ? "text-right" : "text-left"}`}
    >
      <div
      ...
      />
      <div
        className={`relative z-10 flex items-center justify-between px-3 py-1 text-xs ${rtl ? "flex-row-reverse" : ""}`}
      >
