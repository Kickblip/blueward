...
const getDisplayStatus = (
  ...
  status === "RESOLVED" || status === "CANCELLED"
    ? status
    : locksAt && new Date(locksAt).getTime() <= nowMs
      ? "LOCKED"
      : "OPEN"
...
export default function AdminClient() {
  ...
  const sortedMarkets = useMemo(
    () =>
      markets
        .filter((m) => m.status !== "CANCELLED" && m.status !== "RESOLVED")
        .sort((a, b) => b.id - a.id),
    [markets]
  )
    ...
    <div className="grid gap-3 rounded-md border border-zinc-700 bg-zinc-900 p-2">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-md px-2 py-1 text-xs font-semibold uppercase transition-colors duration-200 hover:text-zinc-400"
        >
        ...
        <>
          <div className="grid gap-2 md:grid-cols-[1.4fr_1fr_1fr_auto]">
            ...
            <button
              ...
              disabled={isCreating}
              className="rounded-md bg-blue-600 px-3 py-2 text-sm transition-colors duration-200 hover:bg-blue-500 disabled:opacity-50"
            >
          ...
          {error && (
            <div className="mt-2 rounded-md bg-red-950 px-3 py-2 text-sm text-red-300">
              {error}
            </div>
          )}
